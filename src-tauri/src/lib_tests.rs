...
use crate::commands::diagnostics::{
    ...
    settings_store_path, write_panic_report,
};
...
use crate::commands::fs::{
    calculate_file_hash, decompress_file, detect_mime, dir_size_recursive,
    dir_size_recursive_with_progress, directory_signature_for_path, duplicate_as_alias,
    estimate_dirs_size_fast, format_size, get_dir_size, get_file_info, is_sensitive_for_preview,
    list_directory_entries, normalize_open_with_options, normalize_selected_application_path,
    read_text_preview, resolve_open_with_application_target, trash_delete_error,
...
use crate::models::{DirectorySizeTaskSnapshot, OpenWithOption};
...
use std::sync::Arc;
    ...
    fn read_last_panic_log_limits_large_files() {
        let temp =
            std::env::temp_dir().join(format!("aether-large-panic-log-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&temp);
...
";
    ...
    fn read_text_preview_rejects_invalid_utf8_with_invalid_path_error() {
        let temp =
            std::env::temp_dir().join(format!("aether-preview-invalid-{}", std::process::id()));
    ...
    fn directory_signature_changes_when_visible_entries_change() {
        let temp =
            std::env::temp_dir().join(format!("aether-dir-signature-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&temp);
        std::fs::create_dir_all(&temp).unwrap();
    ...
    fn get_dir_size_includes_nested_files_and_disk_size_fields() {
        let temp =
            std::env::temp_dir().join(format!("aether-dir-size-nested-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&temp);
    ...
    fn estimate_dirs_size_fast_returns_estimate_for_directory() {
        let temp =
            std::env::temp_dir().join(format!("aether-estimate-size-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&temp);
        std::fs::create_dir_all(&temp).unwrap();
    ...
    fn dir_size_recursive_skips_symlinks_without_following_them() {
        let temp =
            std::env::temp_dir().join(format!("aether-dir-size-symlink-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&temp);
        std::fs::create_dir_all(&temp).unwrap();
    ...
    fn dir_size_recursive_with_progress_updates_running_snapshot() {
        ...
        let summary = dir_size_recursive_with_progress(
            &temp,
            "dir-size-test",
            &state,
    ...
    fn dir_size_recursive_with_progress_stops_when_cancelled() {
        let temp =
            std::env::temp_dir().join(format!("aether-dir-size-cancel-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&temp);
        std::fs::create_dir_all(&temp).unwrap();
        std::fs::write(temp.join("a.txt"), vec![0u8; 1024]).unwrap();
    ...
    fn resolve_open_with_application_accepts_existing_app_bundle_paths() {
        let temp =
            std::env::temp_dir().join(format!("aether-open-with-app-{}", std::process::id()));
        let app_path = temp.join("Demo.app");
        let _ = std::fs::remove_dir_all(&temp);
        std::fs::create_dir_all(&app_path).unwrap();

        assert_eq!(
            resolve_open_with_application_target(&format!("{}/", app_path.to_string_lossy()))
                .unwrap(),

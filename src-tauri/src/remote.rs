...
static SFTP_SESSION_CACHE: OnceLock<Mutex<HashMap<String, SftpSessionCacheEntry>>> = OnceLock::new();
...
fn run_remote_command_with_timeout<T, F>(protocol: &'static str, action: &'static str, task: F) -> Result<T, AppError>
where
    T: Send + 'static,
    F: FnOnce() -> Result<T, AppError> + Send + 'static,
{
    ...
    match receiver.recv_timeout(remote_operation_timeout()) {
        ...
        Err(mpsc::RecvTimeoutError::Disconnected) => Err(AppError::internal(format!("{} {}任务异常结束", protocol, action))),
    }
}
...
fn resolve_remote_socket_addrs(connection: &RemoteConnection, protocol: &str) -> Result<Vec<SocketAddr>, AppError> {
...
fn connect_tcp_with_timeout(connection: &RemoteConnection, protocol: &str) -> Result<TcpStream, AppError> {
    ...
    if matches!(error.kind(), ErrorKind::TimedOut | ErrorKind::WouldBlock) || started_at.elapsed() >= remote_operation_timeout() {
        return Err(remote_timeout_error(protocol, "连接"));
    }
...
fn sftp_connection_signature(connection: &RemoteConnection, credential: Option<&RemoteCredential>) -> String {
    format!(
        "{}|{}|{}|{}|{}|{}|{}",
        connection.host,
        connection.port,
...
fn clear_stale_sftp_session_cache(connection: &RemoteConnection, credential: Option<&RemoteCredential>) {
    if let Ok(mut cache) = sftp_session_cache().lock() {
...
fn write_store(app: &tauri::AppHandle, remote_store: &RemoteConnectionsStore) -> Result<(), AppError> {
    let store = app
        .store(REMOTE_CONNECTIONS_FILE)
        .map_err(|e| store_error("无法打开远程连接 Store", e))?;
...
fn parse_remote_path(path: &str) -> Option<RemotePathParts> {
    ...
    Some(RemotePathParts { connection_id, remote_path })
}
...
fn keychain_private_key_passphrase_entry(connection_id: &str) -> Result<Entry, AppError> {
    Entry::new(KEYCHAIN_SERVICE, &keychain_private_key_passphrase_account(connection_id))
        .map_err(|e| AppError::unavailable(format!("无法访问 Keychain: {}", e)))
}
...
fn set_private_key_passphrase(connection_id: &str, passphrase: Option<&str>) -> Result<bool, AppError> {
...
fn get_connection_password(connection_id: &str) -> Result<Option<String>, AppError> {
    match keychain_entry(connection_id)?.get_password() {
        ...
        Err(e) => Err(AppError::unavailable(format!("无法读取远程连接密码: {}", e))),
    }
}
...
fn get_private_key_passphrase(connection_id: &str) -> Result<Option<String>, AppError> {
    match keychain_private_key_passphrase_entry(connection_id)?.get_password() {
        ...
        Err(keyring::Error::NoEntry) => Ok(None),
        Err(e) => Err(AppError::unavailable(format!("无法读取 SFTP 私钥口令: {}", e))),
    }
}
...
fn delete_connection_password(connection_id: &str) -> Result<(), AppError> {
    match keychain_entry(connection_id)?.delete_credential() {
        ...
        Err(e) => Err(AppError::unavailable(format!("无法删除远程连接密码: {}", e))),
    }
}
...
fn delete_private_key_passphrase(connection_id: &str) -> Result<(), AppError> {
    match keychain_private_key_passphrase_entry(connection_id)?.delete_credential() {
        Ok(()) | Err(keyring::Error::NoEntry) => Ok(()),
        Err(e) => Err(AppError::unavailable(format!("无法删除 SFTP 私钥口令: {}", e))),
    }
}
...
fn build_connection_id(input: &SaveRemoteConnectionInput, now: u64) -> String {
    input.id.as_ref().map(|id| id.trim().to_string()).filter(|id| !id.is_empty()).unwrap_or_else(|| {
...
fn normalize_remote_auth_method(protocol: &RemoteProtocol, auth_method: Option<RemoteAuthMethod>) -> RemoteAuthMethod {
...
fn normalize_private_key_path(protocol: &RemoteProtocol, auth_method: &RemoteAuthMethod, path: Option<&str>) -> Result<Option<String>, AppError> {
...
fn expand_private_key_path(path: &str, home_dir: Option<&str>) -> PathBuf {
    if path == "~" {
        return home_dir.map(PathBuf::from).unwrap_or_else(|| PathBuf::from(path));
    }
...
fn normalize_connection_input(input: SaveRemoteConnectionInput, existing: Option<&RemoteConnection>, now: u64) -> Result<RemoteConnection, AppError> {
    ...
    let private_key_path = normalize_private_key_path(&input.protocol, &auth_method, input.private_key_path.as_deref())?;
    ...
    let existing_has_private_key_passphrase = existing.map(|item| item.has_private_key_passphrase).unwrap_or(false);
    let has_private_key_passphrase = if auth_method == RemoteAuthMethod::PrivateKey && input.private_key_passphrase.is_some() {
        !input.private_key_passphrase.as_deref().unwrap_or("").is_empty()
    ...
    Ok(RemoteConnection {
        ...
        host,
        port,
        username: input.username.map(|value| value.trim().to_string()).filter(|value| !value.is_empty()),
...
pub fn save_remote_connection(
    ...
    let now = now_millis();
    let existing = input.id.as_ref().and_then(|id| store.connections.iter().find(|item| &item.id == id));
    ...
    match next.auth_method {
        ...
        RemoteAuthMethod::PrivateKey => {
            ...
            if input.private_key_passphrase.is_some() {
                next.has_private_key_passphrase = set_private_key_passphrase(&next.id, input.private_key_passphrase.as_deref())?;
            } else {
                next.has_private_key_passphrase = existing.map(|item| item.has_private_key_passphrase).unwrap_or(false);
            }
        }
    }
...
#[tauri::command]
pub fn delete_remote_connection(app: tauri::AppHandle, connection_id: String) -> Result<(), AppError> {
    let mut store = read_store(&app)?;
...
fn load_connection(app: &tauri::AppHandle, connection_id: &str) -> Result<RemoteConnection, AppError> {
    read_store(app)?
        .connections
        .into_iter()
        .find(|item| item.id == connection_id)
        .ok_or_else(|| AppError::new(crate::AppErrorKind::NotFound, "远程连接不存在", Some(connection_id.to_string())))
}

#[tauri::command]
pub fn test_remote_connection(app: tauri::AppHandle, connection_id: String) -> Result<(), AppError> {
...
fn test_connection_credential(
    ...
    match connection.auth_method {
        RemoteAuthMethod::Password => {
            ...
            Ok(RemoteCredential::Password(get_connection_password(&connection.id)?))
        }
        RemoteAuthMethod::PrivateKey => {
            ...
            Ok(RemoteCredential::PrivateKeyPassphrase(get_private_key_passphrase(&connection.id)?))
        }
    }
}

#[tauri::command]
pub fn test_remote_connection_input(app: tauri::AppHandle, input: SaveRemoteConnectionInput) -> Result<(), AppError> {
    run_remote_command_with_timeout("远程服务器", "测试连接", move || {
...
pub async fn list_remote_directory(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        run_remote_command_with_timeout("远程服务器", "加载目录", move || {
            ...
            list_remote_directory_impl(&connection, &remote_path, input.show_hidden, Some(&credential))
        })
    })
        .await
...
fn load_connection_credential(connection: &RemoteConnection) -> Result<RemoteCredential, AppError> {
    match connection.auth_method {
        RemoteAuthMethod::Password => Ok(RemoteCredential::Password(get_connection_password(&connection.id)?)),
        RemoteAuthMethod::PrivateKey => Ok(RemoteCredential::PrivateKeyPassphrase(get_private_key_passphrase(&connection.id)?)),
    }
}
...
fn list_remote_directory_impl(
    ...
    match connection.protocol {
        RemoteProtocol::Sftp => list_sftp_directory(connection, remote_path, show_hidden, credential),
        RemoteProtocol::Ftp => list_ftp_directory(connection, remote_path, show_hidden, credential.and_then(|item| match item {
            RemoteCredential::Password(password) => password.as_deref(),
            RemoteCredential::PrivateKeyPassphrase(_) => None,
        })),
        RemoteProtocol::WebdavHttps | RemoteProtocol::WebdavHttp => {
            list_webdav_directory(connection, remote_path, show_hidden, credential.and_then(|item| match item {
                RemoteCredential::Password(password) => password.as_deref(),
                RemoteCredential::PrivateKeyPassphrase(_) => None,
            }))
        }
...
fn remote_file_entry(connection_id: &str, name: String, remote_path: String, is_dir: bool, size: Option<u64>, modified: String) -> FileEntry {
    remote_file_entry_with_type(connection_id, name, remote_path, is_dir, size, modified, None)
}
...
fn remote_file_entry_with_type(
    ...
    FileEntry {
        ...
        is_dir,
        size: if is_dir { "--".into() } else { format_size(size.unwrap_or(0)) },
        modified,
        created: String::new(),
        added: String::new(),
        ...
        file_type: file_type_override
            ...
            .unwrap_or_else(|| if is_dir { "folder".into() } else { crate::detect_mime(&name, false) }),
        icon_path: None,
        child_count: None,
    }
}
...
fn test_sftp_connection_with_client(
    ...
    client.session.set_timeout(remote_io_step_timeout().as_millis() as u32);
    let _ = client.session.keepalive_send();
    let base_path = normalize_remote_directory_path(&connection.base_path);
    let stat = client.sftp.stat(Path::new(&base_path))
        .map_err(|e| map_sftp_error("检查起始目录", e))?;
    if !stat.is_dir() {
        return Err(AppError::invalid_path("SFTP 起始路径不是目录", Some(base_path)));
    }
    Ok(())
}
...
fn get_cached_sftp_client(connection: &RemoteConnection, credential: Option<&RemoteCredential>) -> Option<SftpClientHandle> {
    ...
    if entry.signature != signature || entry.last_used.elapsed() > Duration::from_secs(SFTP_SESSION_IDLE_TTL_SECS) {
        cache.remove(&connection.id);
        return None;
    }
...
fn cache_sftp_client(connection: &RemoteConnection, credential: Option<&RemoteCredential>, client: SftpClient) {
    if let Ok(mut cache) = sftp_session_cache().lock() {
        cache.insert(connection.id.clone(), SftpSessionCacheEntry {
...
fn connect_sftp_client(
    ...
    let mut session = Session::new()
        .map_err(|e| AppError::unavailable(format!("无法创建 SFTP 会话: {}", e)))?;
    ...
    match connection.auth_method {
        ...
        RemoteAuthMethod::PrivateKey => {
            ...
            session
                .userauth_pubkey_file(username, None, &private_key_path_for_auth(private_key_path), passphrase)
                .map_err(|e| map_sftp_error("私钥登录", e))?;
        }
    }
    let sftp = session.sftp()
        .map_err(|e| map_sftp_error("打开文件系统", e))?;
    Ok(SftpClient { session, sftp: Arc::new(sftp) })
}
...
fn list_sftp_directory_with_client(
    ...
    client.session.set_timeout(remote_io_step_timeout().as_millis() as u32);
    let _ = client.session.keepalive_send();
    let rows = client.sftp.readdir(Path::new(remote_path))
        .map_err(|e| map_sftp_error("读取目录", e))?;
    let entries = rows.into_iter().filter_map(|(path, stat)| {
        ...
        let file_type_override = if !is_dir && !stat.is_file() { Some("remote-unknown") } else { None };
...
fn list_ftp_directory(
    ...
    let mut ftp = FtpStream::connect_with_stream(tcp)
        ...
        .passive_stream_builder(move |addr| {
            let stream = TcpStream::connect_timeout(&addr, passive_timeout).map_err(FtpError::ConnectionError)?;
            stream.set_read_timeout(Some(passive_timeout)).map_err(FtpError::ConnectionError)?;
            stream.set_write_timeout(Some(passive_timeout)).map_err(FtpError::ConnectionError)?;
            Ok(stream)
        });
    configure_tcp_stream_timeout(ftp.get_ref(), "FTP")?;
    ftp.login(connection.username.as_deref().unwrap_or("anonymous"), password.unwrap_or("anonymous@"))
        .map_err(|e| map_ftp_error("登录", e))?;
    ftp.cwd(remote_path)
        .map_err(|e| map_ftp_error("进入目录", e))?;

    if let Ok(rows) = ftp.mlsd(None) {
        let entries = rows.into_iter().filter_map(|line| {
            ...
            let size = if is_dir { None } else { Some(file.size() as u64) };
            Some(remote_file_entry(&connection.id, name, child_path, is_dir, size, String::new()))
        }).collect();
        let _ = ftp.quit();
    ...
    let names = ftp.nlst(None)
        .map_err(|e| map_ftp_error("读取目录", e))?;
...
fn connection_base_url(connection: &RemoteConnection) -> String {
    ...
    format!("{}://{}{}", connection.protocol.scheme(), connection.host, port)
}
...
fn list_webdav_directory(
    ...
    if !status.is_success() {
        return Err(AppError::unavailable(format!("WebDAV 服务器返回 {}", status)));
    }
    let text = response
        .text()
...
pub fn parse_webdav_propfind_with_base(
    ...
    responses.into_iter().filter_map(|item| {
        ...
        let name = if !item.display_name.trim().is_empty() {
            ...
            item.href.trim_end_matches('/').rsplit('/').next().unwrap_or("").to_string()
        };
        if !include_remote_name(&name, show_hidden) {
            return None;
        }
        Some(remote_file_entry(connection_id, name, remote_path, item.is_collection, item.size, item.modified))
    }).collect()
}

#[cfg(test)]
fn parse_webdav_propfind(connection_id: &str, requested_path: &str, xml: &str, show_hidden: bool) -> Vec<FileEntry> {
...
mod tests {
    ...
    fn parses_remote_paths() {
        assert_eq!(parse_remote_path("aether-remote://conn-1/sites/public"), Some(RemotePathParts {
    ...
    fn builds_stable_private_key_passphrase_account() {
        assert_eq!(keychain_private_key_passphrase_account("abc"), "remote:abc:private-key-passphrase");
    }

    #[test]
    fn remote_connection_timeout_is_short_enough_for_ui_feedback() {
        assert_eq!(remote_operation_timeout(), Duration::from_secs(REMOTE_OPERATION_TIMEOUT_SECS));
    ...
    fn normalizes_sftp_private_key_connection_config_without_passphrase() {
        ...
        let connection = normalize_connection_input(input, None, 123).expect("valid private-key connection");

        assert_eq!(connection.auth_method, RemoteAuthMethod::PrivateKey);
        assert_eq!(connection.private_key_path.as_deref(), Some("~/.ssh/codex_debian"));
    ...
    fn resolves_webdav_request_path_without_repeating_base_path() {
        ...
        assert_eq!(resolve_webdav_request_path(&connection, "/docs"), "/dav/root/docs");
        assert_eq!(resolve_webdav_request_path(&connection, "/dav/root/docs"), "/dav/root/docs");
    }

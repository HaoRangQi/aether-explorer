...
- CI 正式发布必须有 `TAURI_SIGNING_PRIVATE_KEY` 和 `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` secrets，用于生成 updater `.app.tar.gz.sig`。
...
正式 release workflow 依赖 updater 签名材料：

- `TAURI_SIGNING_PRIVATE_KEY` / `TAURI_SIGNING_PRIVATE_KEY_PASSWORD`：生成 updater `.app.tar.gz.sig`。

推 tag 前先确认仓库 secrets 至少包含下面两项：

```bash
gh secret list -R HaoRangQi/aether-explorer
```

如果缺少 `TAURI_SIGNING_PRIVATE_KEY`，release job 会在 `Validate release inputs` 阶段失败并输出：

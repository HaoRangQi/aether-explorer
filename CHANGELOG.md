...
### 修复
- 传输管理器默认只聚焦当前活跃任务：已完成历史任务不再持续堆积展示，避免“每拖一次就多一条任务”的错觉。
- 传输面板在自动关闭前会主动清理已结束任务；空闲状态下仅短暂展示最近完成结果，随后收敛为干净状态。
- 操作历史执行撤销后不再自动关闭面板，支持连续撤销与连续核对结果。
- 撤销链路增加前置校验：源文件是否仍存在、目标目录是否可用、目标路径是否重名冲突；校验失败直接给出不可撤销原因。
- 撤销结果会把失败原因写回原会话，并在历史面板直接显示，避免只看到“撤销失败”但无原因。

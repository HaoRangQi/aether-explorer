...
### ACK-6 未签名 + 无 Notarization — 当前暂缓，不阻塞公益分发

**位置**：
- `src-tauri/tauri.conf.json` — 历史实现中 `"entitlements": null`
- `README.md` / `README_EN.md` 常见问题节 — 已明确未签名构建风险，`xattr` 仅作为高级用户备用方式
...
项目按公益分发维护，商业化、App Store、Developer ID 签名和 notarization 暂不进入当前路线图，也不作为发版阻塞项。

当前要求：
- README 必须清楚标注未签名版本的打开方式与风险，不把它包装成“正式可信分发”。
- 不把“完全磁盘访问已稳定可用”作为卖点；权限状态以实际可读探针为准。
- 如果未来有 Apple Developer Program 条件，再按下面方案补签名链路。

**未来可选方案**：

A. 申请 Apple Developer Program，配 Developer ID 证书。
...
### ACK-7 完全磁盘访问状态检测仍需增强

**位置**：
- `src-tauri/src/lib.rs` 中的 `open_system_settings` 只打开系统设置面板
- README 写"权限检测和引导授权"
- 实际没有任何 TCC 状态检测代码
...
不等待签名公证，先做基于真实目录读取的权限探针和结构化错误提示。签名后再评估更完整的 TCC 状态代理。
...
- `npm run lint:i18n` 当前验证 79 个 locale key、26 个 ExplorerView 用法、12 个 AIRenamePanel 用法、4 个 app diagnostics 用法、7 个 settings diagnostics 用法、15 个 settings backup 用法、29 个 SettingsView 高风险用法和 10 个 shortcut help 用法。
...
- 已新增 `SECURITY.md`：覆盖漏洞披露、支持范围、安全边界和公益未签名分发口径。
...
- `npm run lint:ci-gates` 当前验证 8 个 test gates、8 个 release gates、8 个 local release gates、10 个 npm script 实现、3 个 dependency resolution checks、19 个 CI setup checks、3 个 timeout checks、6 个工作分支触发、1 个 PR 目标触发、6 个 release trigger checks、13 个 release security checks、11 个版本一致性检查和 release integrity 检查。
...
当前分发不以商业化、App Store、Developer ID 签名或 notarization 为前提。发版前必须保证：
...
- [x] ACK-6 未签名分发风险已在 README / 路线图中明确说明，不包装成正式签名应用
...
> 当前版本已经补齐多项 P0/P1 信任红线，但仍应以"公益社区预览版"的口吻分发，不包装成商业级签名应用。
